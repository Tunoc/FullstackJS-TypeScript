...
describe("Verify /gameapi/getPostIfReached", () => {
  ...
  it("Should find team2, since inside range", async function () {
    const newPosition = { "userName": "t1", "password": "secret", "lon": 12.48, "lat": 55.77, "distance": DISTANCE_TO_SEARCH }
    const config = {
      method: 'POST',
      headers: {
    ...
    const result = await fetch(`${URL}/api/gameapi/nearbyplayers`, config).then(r => r.json());
    expect(result.length).to.be.equal(1)
    expect(result[0].name).to.be.equal("Team2")
  })

  it("Should find team2 +team3, since both are inside range", async function () {
    const newPosition = { "userName": "t1", "password": "secret", "lon": 12.48, "lat": 55.77, "distance": 10000 }
    const config = {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
    ...
    let result = fetch(`${URL}/api/gameapi/nearbyplayers`, config).then(r => r.json());
    await expect(result).to.eventually.be.length(2);
  })

  it("Should NOT find team2, since not in range", async function () {
    const newPosition = { "userName": "t1", "password": "secret", "lon": 12.48, "lat": 55.77, "distance": 10 }
    const config = {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
    ...
    let result = fetch(`${URL}/api/gameapi/nearbyplayers`, config).then(r => r.json());
    await expect(result).to.eventually.be.length(0);
  })

  it("Should NOT find team2, since since credentials are wrong", async function () {
    const newPosition = { "userName": "t1", "password": "NotRight", "lon": 12.48, "lat": 55.77, "distance": DISTANCE_TO_SEARCH }
    const config = {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
    ...
    let result = fetch(`${URL}/api/gameapi/nearbyplayers`, config).then(r => r.json());
    await expect(result).to.eventually.have.property("message", "wrong username or password");
    await expect(result).to.eventually.have.property("code", 403);
  })
})
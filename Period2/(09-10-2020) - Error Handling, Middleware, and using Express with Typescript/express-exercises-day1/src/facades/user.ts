...
function dummyReturnPromise<T>(val: T | null, err: string = "Unknown Error", code: number = 500): Promise<T> { //Generic and promise based.
...
export default class UserFacade {
    ...
    public static users: Array<IGameUser> = [
        { name: "Peter Pan", userName: "pp@b.dk", password: "$2a$10$brgY4043c9ouejz/AxR3wuYMUHniIBQuBuWJXjOoVVtC52jRX66ke", role: "user" },//psw: secret
        { name: "Donald Duck", userName: "dd@b.dk", password: "$2a$10$brgY4043c9ouejz/AxR3wuYMUHniIBQuBuWJXjOoVVtC52jRX66ke", role: "user" },//psw: secret
        { name: "admin", userName: "admin@a.dk", password: "$2a$10$brgY4043c9ouejz/AxR3wuYMUHniIBQuBuWJXjOoVVtC52jRX66ke", role: "admin" }//psw: secret
    ];
    ...
    static async checkUser(userName: string, password: string): Promise<boolean> {
        try {
            ...
            let tmp = await bryptCheckAsync(password, user.password);
            return tmp
        } catch (err) {
